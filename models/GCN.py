...
class GCN_shared(torch.nn.Module):
    def __init__(self,  depth, y_input_size, pos_input_size, hidden_size, ib_n):
        super().__init__()
        
        self.encoder=MLP(y_input_size+pos_input_size, hidden_size, hidden_size)
        conv_list1 = []
        for _ in range(depth):
            conv_list1.append(GCNConv(hidden_size, hidden_size))
        self.conv_list1=nn.ModuleList(conv_list1)
        conv_list2 = []
        for _ in range(depth):
            conv_list2.append(GCNConv(hidden_size, hidden_size))
